...
def get_ratio_distances(results, pattern_col, output_cols):
    ...
    for name, group in results.groupby(pattern_col):
        ...
        for a, b in ratio_pairs:
            ...
            match_dict = {"pattern": name, "sum_diffs": sum_diffs}

            for col in output_cols:
                match_dict.update(
                    {f"match_1_{col}": a_match[col], f"match_2_{col}": b_match[col]}
                )
...
st.subheader(
    "Python Scripts for Analysis of Similar Soggetti in Citations: The Renaissance Imitation Mass"
)
st.write(
    "Visit the [CRIM Project](https://crimproject.org) and its [Members Pages] (https://sites.google.com/haverford.edu/crim-project/home)"
)
st.write(
    "Also see the [Relationship Metadata Viewer] (https://crim-relationship-data-viewer.herokuapp.com/) and [Observation Metadata Viewer] (https://crim-observation-data-viewer.herokuapp.com/)"
)
...
@st.cache(allow_output_mutation=True)
def load_corpusbase(mei_list:List):
    corpus = CorpusBase(paths=mei_list, urls=mei_list)
    return corpus
​
...
duration_choice = st.sidebar.radio(
    "Select Actual or Incremental Durations",
    ["Actual", "Incremental@1", "Incremental@2", "Incremental@4"],
)
...
scale_choice = st.sidebar.radio(
    "Select Diatonic or Chromatic", ["Diatonic", "Chromatic"]
)
...
vector_length = st.sidebar.number_input(
    "Enter a Number for the Length of Vector, or use default",
    min_value=1,
    max_value=20,
    value=5,
)

minimum_match = st.sidebar.number_input(
    "Enter a Minimum Number of Matches, or use default",
    min_value=1,
    max_value=20,
    value=3,
)

close_distance = st.sidebar.number_input(
    "If Close Search, then Enter Threshold, or use default",
    min_value=1,
    max_value=20,
    value=2,
)

max_sum_diffs = st.sidebar.number_input(
    "Enter Maximum Durational Differences (Whole Number or Fractional Value), or use defaul",
    min_value=None,
    max_value=None,
    value=2,
)
...
st.sidebar.write(
    "Adjust Time and Melodic Scales, Vectors, Minimum Matches, and Melodic Flex in Steps 2, 3, 4 at left, or use defaults"
)
...
if st.sidebar.button("Run Exact Search"):
    ...
    s1 = st.text_input(
        'Provide filename for melodic pattern match download (must include ".csv")'
    )
...
if st.sidebar.button("Run Exact Search with Duration Filter"):
    ...
    results["pattern_generating_match"] = results["pattern_generating_match"].apply(
        tuple
    )
    ...
    ratio_distances = get_ratio_distances(
        results,
        "pattern_generating_match",
        ["piece_title", "part", "start_measure", "end_measure"],
    )
    ...
    s3 = st.text_input(
        'Provide filename for durational match download (must include ".csv")'
    )
    tmp_download_link = download_link(
        ratios_filtered, s3, "Click here to download your data!"
    )
    st.markdown(tmp_download_link, unsafe_allow_html=True)
...
if st.sidebar.button("Run Close Search with Duration Filter"):
    ...
    tmp_download_link = download_link(
        ratios_filtered, s4, "Click here to download your data!"
    )
    st.markdown(tmp_download_link, unsafe_allow_html=True)
...
st.sidebar.write()
st.sidebar.write(
    "Set Threshold of Durational Differences for Presentation Types.  NB:  This is different from the durational difference threshold set above"
)
st.sidebar.write()
max_sum_diffs_classify = st.sidebar.number_input(
    "Enter Maximum Durational Differences Among Soggetti to be Classified (whole number only)",
    min_value=None,
    max_value=None,
    value=1,
)
...
if st.sidebar.button("Run Classifier with Exact Search"):
    ...
    tmp_download_link = download_link(
        classified_results, s4, "Click here to download your data!"
    )
    st.markdown(tmp_download_link, unsafe_allow_html=True)
